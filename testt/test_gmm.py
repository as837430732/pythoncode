...
# get current tolerance level  得到当前的公差
...
# fill dense alpha list with initial values  初始化alpha
...
# dot products for two dict format vectors  两个字典格式向量的点积
def dot(v1, v2):
    sum = 0.0
    if len(v1) < len(v2):
        for id in v1.keys():
...
# kernel evaluation for two vectors  核函数评估两个向量
def kernel(v1, v2):
    return dot(v1, v2)
...
# retrieve kernel matrix element on i,j  检索核函数集合元素
def ret_Q(q, i, j):
    if i >= j:
        return q[i][j]
    else:
